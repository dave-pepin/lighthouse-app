...
export async function updateSession(request) {
  ...
  // The root route renders the public marketing landing page for a
  // logged-out visitor (app/page.js) — exact match only, since every path
  ...
  const isSetPasswordPage =
    ...
    request.nextUrl.pathname.startsWith("/title/set-password");
  // Privacy Policy, Terms, and the SMS consent/opt-in explainer must stay
  // publicly viewable (no login wall) — required for A2P 10DLC campaign
  // review (Twilio's reviewers need to visit these directly), and
  ...
  if (user && (isLoginPage || isSignupPage)) {
    ...
    if (profile) {
      ...
      // Neither an agent nor (necessarily) a client — a title company
      // contact has no public.users row either, so check for that next
      // before falling back to the client portal.
      const { data: titleCompanyContact } = await supabase
        .from("title_company_contacts")
        .select("id")
        .eq("user_id", user.id)
        .maybeSingle();
      url.pathname = titleCompanyContact ? "/title/portal" : "/client/portal";
    }
    return NextResponse.redirect(url);
  }

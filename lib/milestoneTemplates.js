...
// order, for one stage's stock label list — used when seeding a new
// Journey's milestones from the template.
export function applyTemplateOverrides(labels, overrideRowsForStage) {
  const enabled = labels.filter((label) => isLabelEnabled(label, overrideRowsForStage));

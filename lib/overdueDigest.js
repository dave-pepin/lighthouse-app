...
// Groups due-or-overdue milestones by agent, first dropping any Journey
// an agent has individually paused (journeys.overdue_digest_paused —
// e.g. a listing temporarily off the market, without deleting the
// Journey or touching the digest for anything else), then filtering
// each remaining milestone against that agent's own
// overdue_digest_threshold_days (null = digest off entirely for them),
// then filtering the resulting agents to those who actually have
// something to report, have an email on file, and are past their
...
export function findOverdueDigestRecipients({ journeys, candidateMilestones, agents, now = new Date() }) {
  ...
  for (const m of candidateMilestones || []) {
    ...
    if (!journey) continue;
    if (journey.overdue_digest_paused) continue; // e.g. a listing off the market for now

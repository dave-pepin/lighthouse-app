...
import { notifyTitleCompanyActivated } from "./actions";

// Same flow as app/client/set-password/page.js, redirecting to the title
// company's own portal instead.
export default function TitleSetPasswordPage() {
  ...
  useEffect(() => {
    ...
    if (access_token && refresh_token) {
      ...
      supabase.auth.getUser().then(({ data }) => {
        setReady(!!data?.user);
        if (!data?.user) {
          setError("This link has expired or already been used. Ask the agent to resend it.");
        }
      });
    }
  }, [supabase]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await notifyTitleCompanyActivated();
    } catch (err) {
      Sentry.captureException(err);
    }
    router.push("/title/portal");
    router.refresh();
  };
    ...
    >
      ...
      >
        ...
        <p style={{ fontSize: 13, color: "var(--lh-slate)", marginBottom: 20 }}>
          Set a password to access your document portal.
        </p>
